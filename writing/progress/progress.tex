...
linear nonlinearity ($f(x) = \max\{.1x,x\}$) is applied at each hidden unit.
...
components with standard PCA and use this as the input. This primarily achieves
computational efficiency by decreasing the total number of parameters in the
network. During optimization, networks are usually trained for 30 epochs
(passes over the training data), and at each epoch we randomly permute the
