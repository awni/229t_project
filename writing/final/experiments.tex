...
Figures \ref{mnist_ae_adagrad_lrs},\ref{mnist_ae_nesterov_lrs} and
\ref{mnist_ae_adadelta_lrs} in the appendix demonstrate another nice property
of the decayed version of AdaGrad, namely robustness to learning rate. Notice
that in Figure \ref{mnist_ae_adagrad_lrs} if we do not tune the learning rate
well, AdaGrad will simply not converge and NAG (cf. Figure
\ref{mnist_ae_nesterov_lrs}), while slightly more robust, will take a long
time.  However, as demonstrated in Figure \ref{mnist_ae_adadelta_lrs}, the
decayed AdaGrad enjoys a robustness to a wide range of learning rates,
converging quickly to nearly the same point at the same rate after the initial
stages of learning. 
